<template>
  <li :class="[{'item-root' : isRootItem },{'off' : !isOpen && isRootItem },{'end-list' : !model.depart }]">
    <div @click.self="setOpen" :class="{on: model.depart && isOpen}">
      <span>
        <i v-if="model.depart" class="fa fa-caret-right" :class="{'rotate' : isOpen}">
        </i>
    ...
    <transition @enter="enter" @leave="leave">
      <ul v-show="isOpen" v-if="model.depart">
        <tree v-for="(model, i) in model.depart" :key="model.id" :model="model" :index="i" :treeEdit="treeEdit">
        </tree>
      </ul>
    </transition>
  </li>
...
export default {
  ...
  computed: {
    isRootItem() {
      return this.model.name === "전사";
    },
    isFolder() {
      return this.model.depart && this.model.depart.length;
    }
  },
  components: {},
  watch: {},
  methods: {
    ...
    removeTree(model) {
      const tree = this.$parent.model.depart;
      tree.splice(tree.indexOf(model), 1);
      //localStorage.setItem(STORAGE_KEY, JSON.stringify(this.todos));
    },
    changeType() {
      if (!this.isFolder) {
        this.$set(this.model, "depart", [{ name: "이름을 수정하세요." }]);
      }
    },
    cancelEdit() {
      this.rename = true;
