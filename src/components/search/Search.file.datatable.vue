<template>
  <section class="template-table-wrap file-data-table">
      ...
      <div class="btn-wrap">
        ...
        <div class="view-check">
          ...
          <el-checkbox-group v-model="view" v-if="morebtn" @change="viewCheck">
            <el-checkbox v-for="(check,i) in definition.field" :label="i" :key="check.i" :ref="'checked'" v-if="i !== 0">
              {{check}}
            </el-checkbox>
          </el-checkbox-group>
        </div>
    ...
    <div class="template-table dynamic-row">
      <div class="table-head-wrap">
        <table>
          <thead>
            <tr>
              <th v-for="(th, i) in definition.field" :key="th.id" :class="['col'+i,{ 'col-end' : definition.field.length-1 === i }]" :ref="'checkedTh'">
                <span>{{th}}</span>
              </th>
            </tr>
          </thead>
    ...
    <el-pagination @size-change="sizeChange" @current-change="currentChange" :current-page.sync="pagination.currentPage" :page-sizes="[25, 50, 100, 200, 500]" :page-size="pagination.size" layout="sizes, prev, pager, next" :total="pagination.total">
    </el-pagination>
  </section>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    getData(page = null, size = null) {
      ...
      this.$http
        .get(this.definition.url, { params: this.searchOption })
        .then(result => {
  ...
  created() {
    if (
      ...
      this.definition.order.length > 0
    ) {
