<template>
  <section class="template-table-wrap">
    <header>
      <span>전체 : {{propData && propData.data && propData.data.metrics ? propData.data.metrics.resultCount : '-'}}건</span>
      <div class="btn-wrap">
        ...
        <div class="view-check">
          ...
          <!--<el-checkbox v-for="(check,i) in propData.field" :label="check" :key="check" :ref="'checked'"-->
          <!--v-if="i !== propData.field.length -1">{{check}}-->
          <!--</el-checkbox>-->
          <!--</el-checkbox-group>-->
        </div>
      </div>
    ...
    <div class="template-table dynamic-row">
      <div class="table-head-wrap">
        <table>
          <thead>
            <tr>
              <th v-for="(th, i) in propData.field" :key="th.id" :class="['col'+i,{ 'col-end' : propData.field.length-1 === i }]" :ref="'checkedTh'">{{th}}
              </th>
            </tr>
          </thead>
        </table>
      ...
      <div class="table-body-wrap">
        <table>
          <tbody>
            <template v-for="(row,i) in propData.data.rows">
    ...
    <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page.sync="page" :page-sizes="[25, 50, 100, 200]" :page-size="length" layout="sizes, prev, pager, next" :total="total">
    </el-pagination>
  </section>
</template>
<script>
  ...
  methods: {
    ...
    rowRoute(val) {
      this.$router.push({
        ...
        query: { ProcessGuid: val.ProcessGuid, nodeid: val.nodeid }
      });
    },
    moreRow(row) {
    ...
    handleSizeChange(val) {
      this.length = val;
      this.$bus.$emit("process-page-length-change", {
        page: this.page,
        length: this.length
      });
    },
