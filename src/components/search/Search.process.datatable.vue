<template>
	<section class="template-table-wrap">
		...
		<div class="template-table dynamic-row">
			<div class="table-head-wrap">
				<table>
					<thead>
					<tr>
						<th v-for="(th, i) in propData.field" :key="th.id"
						    :class="['col'+i,{ 'col-end' : propData.field.length-1 === i }]" :ref="'checkedTh'">{{th}}
						</th>
					</tr>
					</thead>
				</table>
		...
		<el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
		               :current-page.sync="page" :page-sizes="[25, 50, 100, 200]" :page-size="length"
...
//      collength: this.propData.field.length,
    ...
    methods: {
      ...
      handleSizeChange(val) {
        this.length = val;
        this.$bus.$emit('process-page-length-change', {page: this.page, length: this.length});
      },
