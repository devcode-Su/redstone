  ...
  <section class="template-table-wrap">
    <header>
      ...
      <div class="btn-wrap">
        ...
        <el-select v-model="order" placeholder="정렬" size="small" @change="reorder" :disabled="(!propData || !propData.search ||  propData.search.length == 0)">
        ...
        <div class="view-check">
          ...
          <el-checkbox-group v-model="view" v-if="morebtn" @change="viewCheck">
            <el-checkbox v-for="(check,i) in field" :label="check" :key="check" :ref="'checked'" v-if="i !== field.length -1">{{check}}
            </el-checkbox>
          </el-checkbox-group>
        </div>
      </div>
    ...
    <div class="template-table dynamic-row">
      <div class="table-head-wrap">
        <table>
          <thead>
            <tr>
              <th v-for="(th, i) in head" :key="th.id" :class="['col'+i,{ 'col-end' : head.length-1 === i }]" :ref="'checkedTh'">{{th}}
              </th>
            </tr>
          </thead>
        </table>
        ...
        <table :ref="'table'">
          <tbody :ref="'table'">
            <template v-for="(row,i) in viewData">
              <tr :ref="'trs'" :key="row.id" :id="'row' + (row.EventSeq||'0')" :class="[row.detect ? row.detect.Type : '', {'founded': row.isFounded, 'selected': row.isSelected}]">
                ...
                <td class="col2">
                  ...
                  <template v-else-if="row.Table === 'REGISTRY_LOG'">
                    ...
                    <template v-else-if="row.EventType === 'RegSetValue'">
                      {{row.RegKeyPath}} [{{row.RegValueName}}] => [{{row.RegValue}}]
                    </template>
                  ...
                  <template v-else-if="row.Table === 'NETWORK_CONNECT_LOG'">
                    {{row.Direction === 'OUT' ? ' >> ' : '
                    << '}}{{row.Domain ? row.Domain : row.RemoteIp}}:{{row.RemotePort}}/{{row.Protocol}}
								</template>
  ...
  methods: {
    filterTableMapping(filterName, tableName) {
      const map = {
        ...
        network: ["NETWORK_CONNECT_LOG", "NETWORK_DISCONNECT_LOG"],
