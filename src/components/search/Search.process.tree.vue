  ...
  <section class="process-tree">
    ...
    <div class="pc-info">
      <transition-group tag="ul" class="info-list-wrap" name="infolist">
        <li class="infolist" v-for="(list, index) in list" :key="index" ref="infoMenu" :class="{'on' : index === selected}">
...
export default {
        ...
        [
          ...
          { title: "IntegrityLevel", valueName: "IntegrityLevel" },
          {
            ...
            valueName: function(data) {
              return this.hasInteractive(data) ? "있음" : "없음";
            }.bind(this),
            translate: true
          },
          {
  ...
  methods: {
      ...
      selection
        ...
        .on("wheel.zoom", () => {
          ...
          selection.call(
            zoom.transform,
            d3.zoomIdentity
              .translate(this.tree.zoom.x, this.tree.zoom.y)
        ...
        .on("dblclick.zoom", () => {
          ...
          this.tree.zoom.x =
            d3.event.offsetX -
            (d3.event.offsetX - this.tree.zoom.x) * currentPosition.k;
          this.tree.zoom.y =
            d3.event.offsetY -
            (d3.event.offsetY - this.tree.zoom.y) * currentPosition.k;
        ...
        .on("contextmenu", () => {
          ...
          this.tree.zoom.x =
            d3.event.offsetX -
            (d3.event.offsetX - this.tree.zoom.x) *
              this.tree.zoom.k /
              currentPosition.k;
          this.tree.zoom.y =
            d3.event.offsetY -
            (d3.event.offsetY - this.tree.zoom.y) *
              this.tree.zoom.k /
              currentPosition.k;
      ...
      if (tree) {
        if (tree.id === processGuid) {
          ...
          for (let i = 0; i < tree.children.length; i++) {
            tree.children[i] = this.checkSelectedNode(
              processGuid,
              tree.children[i]
            );
          }
    ...
    diagonal(data) {
      let s = data.source;
      let d = data.target;
      return `M${s.y},${s.x}C${(s.y + d.y) / 2},${s.x} ${(s.y + d.y) /
        2},${d.x} ${d.y},${d.x}`;
    },
    update() {
      ...
      let treeMap = d3
        .tree()
        .nodeSize([20, this.tree.viewer.h])
        .separation((a, b) => {
      ...
      nodes.forEach(d => {
        d.y =
          d.depth *
          ((this.tree.viewer.w -
            (this.tree.margin.left + this.tree.margin.right)) /
      ...
      this.moveToParent(nodeExit.transition().duration(duration)).remove();
